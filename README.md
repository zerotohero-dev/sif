...
You can, for example, call [`sif find tutorial`](#searching) and `sif` will display you a list of tutorial links.

`sif`'s main focus is on sifting through and finding links. You can think of it as a command-line bookmark manager.

[Read the usage examples section](#usage-examples) to learn more about what `sif` is capable of.

## Dependencies

`sif` is a [Node.JS][nodejs] command line application. So it requires a [Node.JS][nodejs] runtime.

To install `sif`, you will also need [npm][npm], which comes bundled with [Node.JS][nodejs] most of the time.

`sif` runs best in unix-like environments: It leverages the already superb file searching and sorting capabilities of unix, mainly [`egrep`][egrep] and [`sort`][sort].

> **Note**
>
> `egrep` and `sort` can be implemented in pure JavaScript (as streams) too; [implementing these in the future][issue-16], will make `sif` more portable and more platform-agnostic. Until then if you don't have [bash][bash], then you'll need an emulation layer to run `sif`.

## Supported Operating Systems

### Unix-Like Operating Systems

`sif` uses builtin [bash][bash] commands. So, to use it, you will either need a unix flavor such as [Ubuntu][ubuntu] or [Fedora][fedora]; or a fake linux like [Mac OS][mac].
...
> *Caveat*
>
> `sif` does not work on Windows right now.
>
> Even if you use a linux emulation layer like [Cygwin (*for Windows*)][cygwin],
> you will have problems.

If you use `npm install sif -g` on a windows machine, your installation will be rejected with an error. Even if you use `npm install sif -g --force` you will have problems after the installation.

There are [open issues][issues] about `sif` in Windos, so it'll be fixed **some time** in the future.

If you feel like you can fix it faster, and if you are willing to fix those problems, we are more than happy to integrate your changes into the repo; [just create a pull request for that][git-pr].
...
>
> [Upgrading will be easier in the upcoming versions][issue-68]; but, it's not without side effects right now.

## Configuration

Currently, there are no configuration options for `sif`; [this will change in the upcoming releases][issue-69].
...
Calling `sudo sif update` will update the index file, amending necessary metadata if required.
...
For any issues that you stumble upon, [feel free to open a ticket][new-issue], or [send and email][volkan].
...
> search within arbitrary text data (*[such as runbooks](#the-directory-structure)*); however, the current focus of `sif` being the best program to index bookmarks locally.

## Who Should Use `sif`?

If you love **links**; if you have hundreds (*if not thousands*) bookmarks waiting to be organized; if you have used services like [pocket][pocket], [xmarks][xmarks], or [delicious][delicious] for a hope they will be good enough to sift through your links; if you love to keep your information all together, if you want to **find things without hassle**; and if you like the **speed**, **beauty**, and **simplicity** of the **command line**… then `sif` is just for you.
...
To develop `sif` locally, first [fork it][git-fork] and then [clone][git-clone] it to your development environment.
...
>
> However, [`npm link`][npm-link] will permanently replace your global `sif` command whereas `./devbin/alias.sh` will only replace it for the development session.
...
> **Note**
>
> You can read more about **ES6** and transpilation at [Babel's documentation][babel].

## `sif` Internals

`sif` uses Linux [bash][bash] file processing commands, [child processes][child-process] and [streams][node-streams] to get the job done. It's a command line shell that manages a large text file.
...
Additionally, you are more than welcome to add links to it and [create a pull request][git-pr] to get them merged back.
...
[git-clone]: https://git-scm.com/docs/git-clone "Cloning a Repo"
[git-fork]: https://help.github.com/articles/fork-a-repo/ "Forking a Repo"
...
[xmarks]: http://www.xmarks.com "XMarks"
[delicious]: https://delicious.com "Delicious"

[ubuntu]: http://www.ubuntu.com "Ubuntu"
[fedora]: https://getfedora.org "Fedora"
[cygwin]: https://www.cygwin.com "cygwin"
[mac]: http://www.wikiwand.com/en/Mac_OS "Mac OS"

[bash]: https://www.gnu.org/software/bash/ "Bash"
[egrep]: http://ss64.com/bash/egrep.html "Egrep"
[sort]: http://ss64.com/bash/sort.html "Sort"
[child-process]: https://nodejs.org/api/child_process.html "Child Process"
[node-streams]: https://nodejs.org/api/stream.html "Streams"
[npm-link]: https://docs.npmjs.com/cli/link "npm link"

[semver]: http://semver.org "Semantic Versionung"

[issues]: http://github.com/v0lkan/sif/issues/ "Issues"
[new-issue]: http://github.com/v0lkan/sif/issues/new "Create a new issue"
